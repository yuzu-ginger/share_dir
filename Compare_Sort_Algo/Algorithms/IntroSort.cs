...
namespace Compare_Sort_Algo.Algorithms
{
    ...
    internal class IntroSort : SortBase
    {
        ...
        public override void Sort(int[] array, Counter counter)
        {
            ...
            IntroSortRange(array, 0, array.Length - 1, depthLimit, counter);
        }
        ...
        /// <param name="counter">比較回数、再帰をカウント</param>
        private void IntroSortRange(int[] array, int left, int right, int depthLimit, Counter counter)
        {
            ...
            IntroSortRange(array, left, j, depthLimit - 1, counter);   // pivot 以下の領域
            IntroSortRange(array, i, right, depthLimit - 1, counter);  // pivot 以上の領域
